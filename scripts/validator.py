...
def get_defaults(config_type: ConfigType) -> dict:
    """加载指定类型的默认值"""
    defaults_path = Path(config_type.value) / "_defaults.yaml"
...
def validate_references(change: Change, data: Optional[dict]) -> list[str]:
    ...
    if change.config_type == ConfigType.SERVICES:
        ...
        for member in members:
            host_path = Path("hosts/config") / member
            group_path = Path("host_groups/config") / member
...
def _resolve_ref(ref: str) -> Optional[Path]:
    """解析引用，返回配置路径（如果存在）"""
    # 先当作 host 查找
    host_path = Path("hosts/config") / ref
    if host_path.exists():
        return host_path

    # 再当作 host_group 查找
    group_path = Path("host_groups/config") / ref
